...
(function () {
  ...
  function freshState() {
    return {
      ...
      round: null,
      clues: ['', '', '', ''],
      guess: { slots: [null, null, null, null], tray: [], trayScatter: {}, tileRotations: {}, lockedSlots: [false, false, false, false], slotSpin: [0, 0, 0, 0], traySpin: {}, attempts: 0 },
  ...
  /** Rend les 4 côtés d'une tuile. `words` sont TOUJOURS les mots canoniques (rotation 0,
   * l'ordre fixe imprimé sur la tuile) : la tuile ne redessine jamais son texte à l'horizontale
   * dans une nouvelle case selon sa rotation — elle pivote comme un bloc rigide entier via CSS
   * (--tile-rotation sur `.tile-turn`, cœur + carton ensemble). Ça évite le mauvais repli du
   * texte et le « redressement ».
   * `rotation` (0-3) sert uniquement à savoir quel mot canonique se retrouve, une fois la tuile
   * tournée, sur un côté extérieur du plateau (pour le style .exterior). */
  function tileFaceHTML(words, exteriorSet, opts) {
    opts = opts || {};
    const cls = opts.extraClass || '';
    const rotation = opts.rotation || 0;
    const COMPASS = ['T', 'R', 'B', 'L'];
    const isExterior = (canonicalIndex) => exteriorSet.has(COMPASS[(canonicalIndex + rotation) % 4]);
    return `
      <div class="tile-face ${cls}">
        <div class="side side-top${isExterior(0) ? ' exterior' : ''}">${esc(words.T)}</div>
        <div class="side side-right${isExterior(1) ? ' exterior' : ''}">${esc(words.R)}</div>
        <div class="side side-bottom${isExterior(2) ? ' exterior' : ''}">${esc(words.B)}</div>
        <div class="side side-left${isExterior(3) ? ' exterior' : ''}">${esc(words.L)}</div>
        <div class="side-center">${opts.centerHTML || ''}</div>
      </div>`;
  }

  /** Tuile + petit cœur dans le même bloc tournant : le cœur ne peut plus rester droit
   * pendant que le carton pivote. `turns` est l'angle en quarts de tour (peut dépasser 0-3). */
  function tileBundleHTML(words, exteriorSet, opts) {
    opts = opts || {};
    const turns = opts.turns || 0;
    return `<div class="tile-turn" style="--tile-rotation: ${turns * 90}deg;">
      <div class="tile-heart" aria-hidden="true"></div>
      ${tileFaceHTML(words, exteriorSet, opts)}
    </div>`;
  }
  ...
  function cloverHTML(arrangement, opts) {
    ...
    const hasSlotSpin = Array.isArray(opts.slotSpin);
    let cells = '';
    for (let i = 0; i < 4; i++) {
      const cell = arrangement[i];
      const locked = lockedSlots[i];
      if (cell) {
        // Mots canoniques (rotation 0) : la rotation réelle est appliquée visuellement via
        // --tile-rotation (voir plus haut), jamais en redessinant les mots dans d'autres cases.
        const words = Game.wordsAt(cell.tile, 0);
        let centerHTML = '';
        if (locked) {
        ...
        const turns = hasSlotSpin ? (opts.slotSpin[i] || 0) : (cell.rotation || 0);
        cells += `<div class="clover-cell slot-${i}${opts.interactive && !locked ? ' clickable' : ''}${locked ? ' locked' : ''}${selected ? ' selected' : ''}" data-slot="${i}">
          ${tileBundleHTML(words, EXTERIOR_SIDES[i], { centerHTML, rotation: cell.rotation, turns })}
        </div>`;
      } else {
  ...
  /** Assemble le trèfle. `cornerButtonsHTML` (optionnel) : { home, verify } — boutons
   * HORS du plateau qui pivote, figés à gauche/droite de la pointe du cœur. */
  function boardHTML(arrangement, opts, edgesHTML, cornerButtonsHTML) {
    return `
      <div class="board-wrap">
        ...
        ${cornerButtonsHTML ? tipButtonsRowHTML(cornerButtonsHTML.verify) : ''}
      </div>`;
  }
  ...
  function homeBtnHTML() {
    return `<button class="corner-btn corner-btn-home" data-action="go-home" title="Retour à l'accueil">🏠</button>`;
  }

  function tipButtonsRowHTML(verifyHTML) {
    return `<div class="board-corner-row">
      <div class="board-corner">${homeBtnHTML()}</div>
      <div class="board-corner">${verifyHTML || '<span class="corner-btn-spacer"></span>'}</div>
    </div>`;
  }
  ...
  function render() {
    // Hors plateau de jeu (accueil, passage d'appareil, scores), le cœur de fond
    // reprend toujours l'orientation de départ — il ne doit pas garder la rotation
    // du tableau précédent.
    if (state.screen !== 'cluegiver' && state.screen !== 'guess') {
  ...
  function setupGuessPhase() {
    // Plateau et tuiles repartent à l'endroit : le cœur (grand et petits) pointe vers le haut.
    state.boardRotation = 0;
    applyBoardRotation();
    const rotations = {};
    const traySpin = {};
  ...
  /** Petit décalage aléatoire pour l'effet « en vrac », sans incliner le cœur. */
  ...
  function swapSlots(indexA, indexB) {
    ...
    slots[indexB] = tmp;
    const tmpSpin = state.guess.slotSpin[indexA];
    state.guess.slotSpin[indexA] = state.guess.slotSpin[indexB];
    state.guess.slotSpin[indexB] = tmpSpin;
  }
  ...
  function removeSlotToTray(slotIndex) {
    ...
    state.guess.tray.push(occupant.tileId);
    state.guess.trayScatter[occupant.tileId] = randomScatter();
    state.guess.traySpin[occupant.tileId] = state.guess.slotSpin[slotIndex] || occupant.rotation || 0;
    state.guess.slotSpin[slotIndex] = 0;
  }
  ...
  /** Anime `.tile-turn` jusqu'à un angle absolu (quarts de tour), sans jamais revenir à 0°
   * entre-temps — sinon le petit cœur (et la tuile) clignotent droits. */
  function spinTurnTo(el, toTurns, callback) {
    if (!el) {
      callback();
      return;
    }
    el.style.transition = 'transform 0.35s ease';
    el.style.transform = `rotate(${toTurns * 90}deg)`;
    setTimeout(callback, 350);
  }

  /** Fait pivoter une tuile du tas. L'animation part de l'angle ACTUEL (traySpin) vers le
   * nouveau, jamais d'un 0° implicite — sinon la tuile saute au lieu de continuer sa course. */
  function rotateTrayBy(tileId, steps) {
    const el = app.querySelector(`.tray-tile[data-tile="${tileId}"] .tile-turn`);
    const current = state.guess.traySpin[tileId] || 0;
    const next = current + steps;
    spinTurnTo(el, next, () => {
      state.guess.tileRotations[tileId] = (((state.guess.tileRotations[tileId] || 0) + steps) % 4 + 4) % 4;
      state.guess.traySpin[tileId] = normalizeSlotSpin(next);
      render();
    });
  }
  ...
  /** Fait pivoter le bloc `.tile-turn` (carton + petit cœur) d'un cran, en partant de l'angle
   * déjà affiché — pas de la case entière, qui sinon remettait le cœur à 0° après l'animation. */
  function rotateSlotBy(slotIndex, steps) {
    ...
    const el = app.querySelector(`.clover-cell[data-slot="${slotIndex}"] .tile-turn`);
    spinTurnTo(el, next, () => {
  ...
  function createDragGhost() {
    ...
    const turns = dragState.source.type === 'slot'
      ? (state.guess.slotSpin[dragState.source.slotIndex] || 0)
      : (state.guess.traySpin[dragState.tileId] || 0);
    const words = Game.wordsAt(tile, 0);
    const ghost = document.createElement('div');
    ghost.className = 'drag-ghost';
    ghost.style.width = `${dragState.width}px`;
    ghost.style.height = `${dragState.height}px`;
    ghost.innerHTML = tileBundleHTML(words, new Set(), { rotation, turns });
  ...
  function onPointerDown(e) {
    ...
    if (trayTileEl) {
      ...
      if (cell) {
        ...
        // Case vide : pas de tuile à glisser ; un clic fera pivoter le plateau.
        source = { type: 'slot-empty', slotIndex };
      }
      sourceEl = slotEl;
    } else if (boardEl) {
      // Reste du plateau (bords/indices) : un clic fait pivoter tout le plateau.
      source = { type: 'board' };
      sourceEl = boardEl;
    } else {
      return;
  ...
  function onPointerUp(e) {
    ...
    // Clic (sans glisser) : rotation. Glisser-déposer seul déplace les tuiles.
    if (source.type === 'tray') {
      rotateTrayBy(source.tileId, clickRotatesClockwise(sourceEl, e.clientX) ? 1 : -1);
      return;
    }
    if (source.type === 'slot') {
      rotateSlotBy(source.slotIndex, clickRotatesClockwise(sourceEl, e.clientX) ? 1 : -1);
      return;
    }
  ...
  /** Clic simple sur le plateau (écran donneur d'indices) : pivote tout le plateau.
   * Moitié droite = vers la droite, moitié gauche = vers la gauche. Sur l'écran de
   * devinette, c'est onPointerUp qui gère déjà ce cas. */
  ...
  const ROTATE_CURSOR_LEFT = rotateCursorUrl(false);

  /** Survol des zones de pivotement : le curseur indique le sens (droite/gauche)
   * qu'un clic déclenchera à cet endroit. */
